        ...
        <template #default="scope">
          <div class="button-mgl">
            ...
            <el-button
              ...
              class="button-op-del"
              size="small"
              @click="GenGoodsInOutBillByCheckApi(scope.row.BillId)"
              >生成盘点出入库单
            </el-button>
          </div>
        </template>
...
import { SearchGoodsBills, GenGoodsInOutBillByCheck } from "@/api/bill.js";
...
/////////////////////////////////////////////////////////////////////////////////
//根据库存盘点单生成入库单，出库单
const GenGoodsInOutBillByCheckApi = async (BillID) => {
  let res = await GenGoodsInOutBillByCheck({
    CheckBillId: BillID,
    GenIn: true,
    GenOut: true,
  });
...
watch(
  ...
  (data) => {
    ...
    if (
      ...
      isRenovate == 1 &&
      store.state.tags.pageSaerch.titlePage == "盘点单"
    ) {
      query.PageStartRow = 1;
      getData();
