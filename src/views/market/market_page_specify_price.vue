<template>
    <div class="container">
        <div class="handle-box">
            <el-form label-width="1px">
                <el-row>
                    <el-col :span="5">
                        <el-form-item label="">
                            <el-input v-model="query.search.act_code" clearable placeholder="请输入编码">
                            </el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span="5" :offset="1">
                        <el-form-item label="">
                            <el-input v-model="query.search.act_name" clearable placeholder="请输入名称">
                            </el-input>
                        </el-form-item>
                    </el-col>
                ...
                <el-row>
                    ...
                    <el-col :span="5" :offset="1">
                        <el-form-item label="">
                            <el-select clearable style="width: 100%" v-model="query.search.act_status"
                                placeholder="单据状态">
        ...
        <el-table :data="tableData" border class="table" v-loading="tableLoading" ref="multipleTable"
            ...
            <el-table-column align="center" label="小程序码" width="120">
                <template #default="scope">
                    <img :src="scope.row.WxxcxQRCode" @click="showQrModal(scope.row.WxxcxQRCode)" style="width: 100%" />
                </template>
            </el-table-column>
            ...
            <el-table-column prop="ActName" align="center" label="名称">
                <template #default="scope">
                    <el-link :underline="false" @click="handleEdit(scope.row.ActId, 'View')" style="color: #f56c6c">
            ...
            <el-table-column v-if="query.search.act_type == 2" align="center" label="可使用商品">
                <template #default="scope">
            ...
            <el-table-column label="操作" align="center" width="80">
                <template #default="scope">
                    <div class="button-mgl">
                        <el-button class="button-op-edit" v-if="scope.row.ActStatus == 0" size="small"
                            @click="handleEdit(scope.row.ActId, 'Edit')">编辑
                        </el-button>
                        <el-button v-if="scope.row.ActStatus == 0" class="button-op-view" size="small"
                            @click="OpApproveAndCancel(scope.row.ActId, 'MKApprove')">审批
                        </el-button>
                        <el-button v-if="scope.row.ActStatus == 0" class="button-op-del" size="small"
                            @click="OpApproveAndCancel(scope.row.ActId, 'MKCancel')">取消
                        </el-button>
                        <el-button v-if="scope.row.ActStatus == 1" class="button-op-del" size="small"
                            @click="OpApproveAndCancel(scope.row.ActId, 'MKPause')">暂停
                        </el-button>
                        <el-button v-if="scope.row.ActStatus == 2" class="button-op-reset" size="small"
                            @click="OpApproveAndCancel(scope.row.ActId, 'MKRestore')">恢复
                        </el-button>
                    </div>
                </template>
            </el-table-column>
        ...
        <!-- 操作弹框 -->
        <delDialog ref="editor" :IDs="ActID" :OpType="OpType" :BillType="'MK'" :Visible="OpVisible"
            @RefreshData="RefreshData" @CloseVisible="CloseVisible">
        </delDialog>
        <!-- 查看小程序码 -->
        <QrCodeModal :QrCodeVisible="showModal" :QrCodeUrl="QrCodeValue" :QrTitle="'小程序码'" @CloseVisible="CloseQrVisible">
        </QrCodeModal>
    </div>
</template>
...
const CloseQrVisible = async (BooleanValue) => {
...
watch(
    ...
    (data) => {
        ...
        if (
            isRenovate == 1 &&
            store.state.tags.pageSaerch.titlePage == "指定价格活动"
        ) {
            handleSearch();
