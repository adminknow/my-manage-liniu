...
const Init = () => {
  if (router.currentRoute.value.query.type) {
    query.search.act_type = router.currentRoute.value.query.type;
    getData();
  }
};
Init();
...
watch(
  ...
  (data) => {
    ...
    store.state.tags.tagsList.forEach((row) => {
      if (
        (row.title == "买赠活动" && data.query.type == 1) ||
        (row.title == "满减活动" && data.query.type == 2)
      ) {
        isRenovate = 1;
      }
    });
    ...
    if (isRenovate == 1) {
      let type = data.query.type; //1、买赠 2、满减
      query.search.act_type = type;
      handleSearch();
