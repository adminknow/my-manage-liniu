          ...
          <el-row>
            <el-col :span="23">
              <el-form-item label="上级分类" style="position: relative">
                <el-tree-select
                  v-model="form.ParentGoodsCategoryId"
                  :data="data"
                  ref="treeRef"
                  :check-strictly="true"
                  ...
                  node-key="value"
                  style="width: 76%"
                  placeholder="不选代表为一级分类"
                />
                <el-button
                  type="danger"
                  @click="clearableTreeSel"
            ...
            <el-col :span="23">
              <el-form-item label="详情" prop="GoodsCategoryDetail">
                <el-input
                  ...
                  :rows="4"
                  show-word-limit
                  v-model="form.GoodsCategoryDetail"
                ></el-input>
              </el-form-item>
            </el-col>
          </el-row>
...
const onSubmit = async () => {
  formRef.value.validate(async (valid) => {
    if (valid) {
      ...
      //   return;
      // }
      let res = await SaveGoodsCategory({
        GoodsCategory: form,
        IsSubmit: true,
      });
      btnLoading.value = false;
...
const GetGoodsCategoryApi = async (ID) => {
  ...
  if (res.Code === 200) {
    ...
    form.ParentGoodsCategoryId =
      res.DataMap.ParentGoodsCategoryId == 0
        ? ""
        : res.DataMap.ParentGoodsCategoryId;
...
//根据ID获取详情
const Init = async () => {
  defaultKeys.value = [router.currentRoute.value.query.treeId];
  paraMeter.UnitId = router.currentRoute.value.query.UnitId;
  GetPublicGoodsCategoryListApi();
  if (router.currentRoute.value.query.ID) {
    form.ObjectId = router.currentRoute.value.query.ID;
    Type.value = router.currentRoute.value.query.Type;
    GetGoodsCategoryApi(form.ObjectId);
  }
};
Init();
</script>
