<template>
  <div class="container">
          ...
          <el-col :span="5" :offset="1">
            <el-form-item label="">
              <el-select
                clearable
                style="width: 100%"
                v-model="query.search.goods_src"
                placeholder="请选择商品来源"
              >
          ...
          <el-col :span="5">
            ...
            <el-form-item label="" v-if="loginUser.UnitType == 2">
              <el-select
                clearable
                style="width: 100%"
                v-model="query.search.mall_status_1"
                placeholder="请选择礼牛商城状态"
              >
          ...
          <el-col :span="7">
            <div class="button-mgl">
              <el-button
                ...
                class="button2 button-float"
                round
                @click="fileUploadClick('SellGoods')"
                >批量导入
              </el-button>
              <el-dropdown
                class="button-float"
                v-if="loginUser.UnitName == '礼牛严选' && IsRight2"
              >
                <el-button class="button2 button-float" round
                  >云中鹤导入导出
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    <el-dropdown-item
                      @click="fileUploadClick('HaoxinySellGoods')"
                      >商品批量导入</el-dropdown-item
                    >
                    <el-dropdown-item
                      @click="fileUploadClick('HaoxinyGoodsCategoryMap')"
                      >商品分类映射导入</el-dropdown-item
                    >
                    <el-dropdown-item @click="outTableData(false)"
                      >商品分类映射导出</el-dropdown-item
                    >
                    <el-dropdown-item @click="outTableData(true)"
                      >虚拟商品分类映射导出</el-dropdown-item
                    >
                  </el-dropdown-menu>
                </template>
              </el-dropdown>
    ...
    >
        ...
        <template #default="scope">
          ...
          <div v-if="scope.row.Malls.length > 0">
            <div v-for="(item, index) in scope.row.Malls" :key="index">
              ...
              >
                分类：<span
                  v-for="(item1, index1) in item.Categories"
                  :key="index1"
                  >{{ item1.Code + "-" + item1.PathName }},</span
                >
              </p>
            </div>
          </div>
      ...
      <el-table-column prop="" align="left" label="礼牛商城">
        <template #default="scope">
          <div v-for="(item, index) in scope.row.Malls" :key="index">
            <div v-if="item.UnitId == 1">
              ...
              <div v-if="item.MallStatus == 10">
                <p v-if="item.Price && item.Price.OrgPrice > 0">
                  原价：{{ (item.Price.OrgPrice / 100).toFixed(2) }}(元)
                </p>
                <p v-if="item.Price && item.Price.SellPrice > 0">
                  售价：{{ (item.Price.SellPrice / 100).toFixed(2) }}(元)
                </p>
                <p v-if="item.Price && item.Price.WholesalePrice > 0">
                  满{{ item.Price.WholesaleBeginQty }}件,每件{{
                ...
                <p
                  v-if="
                    item.Categories &&
                    item.Categories.length > 0 &&
                    (item.MallStatus == 10 || item.MallStatus == 0)
                  "
                >
                  分类：<span
      ...
      <el-table-column prop="" align="left" label="福采商城" v-if="isHide == 1">
        <template #default="scope">
          <div v-for="(item, index) in scope.row.Malls" :key="index">
      ...
      <el-table-column label="操作" align="center" width="160">
        <template #default="scope">
          <div class="button-mgl">
            <el-button
              v-if="loginUser.UnitId == scope.row.CreateUnitId && IsRight"
              class="button-op-edit"
              size="small"
              @click="handleEdit(scope.row.GoodsId, 'Edit')"
              >编辑
            </el-button>
            <el-button
              v-if="
              ...
              class="button-op-del"
              size="small"
              @click="handleDisable(scope.row.GoodsId)"
              >停用
            </el-button>
            <el-button
              v-if="
              ...
              size="small"
              @click="handleEnable(scope.row.GoodsId)"
              >启用
            </el-button>
            <el-button
              v-if="
              ...
              class="button-op-del"
              size="small"
              @click="handleAddInventory(scope.row.GoodsId)"
              >增加库存
            </el-button>
            <el-button
              v-if="scope.row.GoodsProperties.length == 0"
              class="button-op-del"
              size="small"
              @click="handleViewInventory(scope.row.GoodsId)"
              >库存变动历史
            </el-button>
            <el-button
              v-if="
              ...
              size="small"
              @click="handleSKU(scope.row.GoodsId, scope.row.CreateUnitId)"
              >规格
            </el-button>
            <el-button
              v-if="loginUser.UnitName == '礼牛网'"
            ...
            <el-button
              class="button-op-view"
              size="small"
              @click="goodsUrl(scope.row.GoodsId)"
              >商品链接
            </el-button>
            <el-button
              class="button-op-view"
              size="small"
              @click="Evaluate(scope.row.GoodsId)"
              >评价
            </el-button>
          </div>
        </template>
    ...
    >
      ...
      >
        ...
        >
          <template #default="scope">
            <el-link
              :underline="false"
              @click="handleTicketEdit(scope.row.TicketTemplateId)"
              style="color: #f56c6c"
            >
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button class="button2" @click="ticketVisible = false"
            >关 闭</el-button
          >
        </span>
      </template>
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button
            ...
            @click="saveInventoryBtn"
            >保 存</el-button
          >
          <el-button class="button2" @click="inventoryVisible = false"
            >关 闭</el-button
          >
        </span>
      </template>
    ...
    <el-dialog
      class="inventory-top"
      title="库存变更历史"
      v-model="inventoryHistoryVisible"
      width="60%"
      :show-close="false"
    >
      <goodsQty
        ...
        :goodID="goodID"
        :skuID="skuID"
        :openModal="inventoryHistoryVisible"
      >
      </goodsQty>
      <template #footer>
        <span class="dialog-footer">
          <el-button class="button2" @click="inventoryHistoryVisible = false"
            >关 闭</el-button
          >
        </span>
      </template>
    ...
    <GoodsLink
      :GoodsLinkVisible="GoodsLinkVisible"
      :link="url1"
      :link2="url2"
      :link3="url3"
      @CloseVisible="CloseGoodsLinkVisible"
    ></GoodsLink>
    ...
    <evaluate-index
      ...
      @CloseVisible="CloseEvaluateVisible"
    ></evaluate-index>
  </div>
</template>
...
import GoodsLink from "@/components/Modal/GoodsLink.vue";
...
let isHide = ref("");
if (router.currentRoute.value.query.hide) {
  isHide.value = router.currentRoute.value.query.hide;
}
...
let url2 = ref("");
let url3 = ref("");
let GoodsLinkVisible = ref(false);
const goodsUrl = (pID) => {
  url1.value = "";
  url2.value = "";
  url3.value = "";
  for (let i = 0; i < tableData.value.length; i++) {
    const el = tableData.value[i];
    if (el.GoodsId == pID) {
      if (ConfigValue.value == 1) {
        //自有商城
        url1.value =
          "/pages/productDetail/index_new?UnitId=" +
          loginUser.UnitId +
          "&id=" +
          el.GoodsId;
      }
      el.Malls.forEach((row) => {
        if (row.UnitId == 1 && row.MallStatus == 10) {
          //已上架的商品才能显示连接
          url2.value = "/pages/productDetail/index_new?id=" + el.GoodsId;
        }
      });
      break;
    }
  }
  GoodsLinkVisible.value = true;
};
const CloseGoodsLinkVisible = () => {
  GoodsLinkVisible.value = false;
};
////////////////////////////////////////////////////////////////
//导出
const handleOut = () => {
      ...
      if (res.Code == 200) {
        ...
        import("../../vendor/Export2Excel").then((excel) => {
          res.DataMap.Data.forEach((row) => {
            ...
            row.xcxUrlDesc3 = "";
            if (ConfigValue.value == 1) {
              row.xcxUrlDesc1 = `/pages/productDetail/index_new?UnitId=${loginUser.UnitId}&id=${row.GoodsId}`;
            } else {
              row.xcxUrlDesc2 = `/pages/productDetail/index_new?id=${row.GoodsId}`;
            }
            ...
            if (row.Malls.length > 0) {
              ...
              for (let i = 0; i < row.Malls.length; i++) {
                ...
                if (el.UnitId == 1) {
                  ...
                  //状态
                  row.MallStatusLiNiuDesc = "";
                  if (el.MallStatus == 0) {
                    row.MallStatusLiNiuDesc = "待审核";
                  } else if (el.MallStatus == 10) {
                    row.MallStatusLiNiuDesc = "上架中";
                  } else if (el.MallStatus == -1) {
                    row.MallStatusLiNiuDesc = "未上架";
                  }
                } else if (el.UnitId == 2) {
                  //礼牛平台
                  ...
                  //状态
                  row.MallStatusFuCaiDesc = "";
                  if (el.MallStatus == 0) {
                    row.MallStatusFuCaiDesc = "待审核";
                  } else if (el.MallStatus == 10) {
                    row.MallStatusFuCaiDesc = "上架中";
                  } else if (el.MallStatus == -1) {
                    row.MallStatusFuCaiDesc = "未上架";
                  }
                } else {
                  row.CategoriesDesc = "";
                  el.Categories.forEach((CategoriesRow) => {
          ...
          const tHeader = [
            ...
            "礼牛商城状态", //MallStatusLiNiuDesc
            // "福采商城状态", //MallStatusFuCaiDesc
          ...
          if (isHide.value == 1) {
            tHeader.push("福采商城状态");
            filterVal.push("MallStatusFuCaiDesc");
          }
          const data = formatJson(filterVal, res.DataMap.Data);
          excel.export_json_to_excel({
            header: tHeader,
            data: data,
            filename: "自有商城商品数据导出" + time,
            autoWidth: true,
          });
        });
      }
