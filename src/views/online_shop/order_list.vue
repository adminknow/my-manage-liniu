<template>
  <div class="container">
    <div class="handle-box">
      <el-form label-width="1px">
        <el-row>
          <el-col :span="5">
            <el-form-item label="">
              <el-input v-model="query.search.order_code" clearable placeholder="请输入订单号"></el-input>
            </el-form-item>
          </el-col>
          ...
          <el-col :span="5" :offset="1">
            <el-form-item label="">
              <el-input v-model="query.search.order_id" clearable placeholder="请输入订单ID"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6" :offset="1">
            <div class="button-mgl">
              <el-button class="button2 button-float" round @click="handleIn()" v-if="IsApproveOrder">
                <el-icon>
                  <DocumentAdd />
                </el-icon>&nbsp;&nbsp;导入发货
              </el-button>
        ...
        <el-row>
          <el-col :span="5">
            <el-form-item label="">
              <el-input v-model="query.search.ticket_code" clearable placeholder="请输入券码"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="5" :offset="1">
            <el-form-item label="">
              <el-input v-model="query.search.wxxcx_openid" clearable placeholder="请输入OpenID"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="5" :offset="1">
            <el-form-item label="">
              <el-input v-model="query.search.contact_cell" clearable placeholder="请输入收件人电话"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6" :offset="1">
            <div class="button-mgl">
        ...
        <el-row>
          <el-col :span="5">
            <el-form-item label="">
              <el-input v-model="query.search.goods_name" clearable placeholder="请输入商品名称或编码"></el-input>
            </el-form-item>
            <!-- <el-form-item label="">
              <el-input
          ...
          <el-col :span="5" :offset="1">
            <el-form-item label="">
              <el-select clearable style="width: 100%" v-model="query.search.invoice_status" placeholder="请选择发票状态">
          ...
          <el-col :span="5" :offset="1">
            <el-form-item label="" v-if="loginUser.UnitType == 1">
              <el-select v-model="query.search.unit_id" filterable remote reserve-keyword placeholder="请输入券商名称"
    ...
    <div class="order_tags">
      <ul>
        <li @click="choosetag(index)" class="order_tags-li" v-for="(item, index) in statusList" :class="{
            active: item.active,
          ...
          <text v-else-if="item.key == '待审核' && StatData.wait_approve_cnt > 0">({{ StatData.wait_approve_cnt }})</text>
          <text v-else-if="item.key == '待发货' && StatData.wait_deliver_cnt > 0">({{ StatData.wait_deliver_cnt }})</text>
        </li>
      </ul>
    </div>
    <div style="width: 100%; height: 8px"></div>
    <el-table @selection-change="handleSelectionOrderChange" :data="tableData" border class="table"
      v-loading="tableLoading" ref="tableDataRef" :header-cell-style="{ 'text-align': 'center' }"
        ...
        <template #default="props">
          <div class="child-contant">
            ...
            <el-table @selection-change="
              ...
              " :data="props.row.Details" style="width: 90%" border :header-cell-style="{ 'text-align': 'center' }">
              ...
              <el-table-column align="left" label="商品">
                <template #default="scope">
                  <el-popover :width="300"
                    ...
                    <template #reference>
                      <el-link @mouseover="mouseoverGoods(scope.row.GoodsId)" @click="jumpGoodsClick(scope.row.GoodsId)"
      ...
      <el-table-column type="selection" :selectable="selectedApprove" align="center" width="50" />
      <el-table-column align="left" label="订单信息" width="220">
        <template #default="scope">
          ...
          <p>{{ scope.row.AccountName+'--'+scope.row.OrderTypeDesc }}</p>
        </template>
      </el-table-column>
      ...
      <el-table-column align="left" label="物流信息" width="170">
        <template #default="scope">
          <div v-for="(item, index) in scope.row.Expresses" :key="index">
            ...
            <p>
              {{ item.ExpressCode
              }}<span v-if="query.search.order_status == '31,40'"><el-link
      ...
      <el-table-column label="操作" align="center" width="160">
        <template #default="scope">
          <div class="button-mgl">
            <el-button v-if="scope.row.OrderStatus == 20 && IsApproveOrder" class="button-op-edit" size="small"
              @click="handleOrderByApprove(scope.row.OrderId)">审核
            </el-button>
            <el-button v-if="
            ...
            <el-button v-if="
                ...
                IsExpressOrder
              " class="button-op-reset" size="small" @click="handleSend(scope.row.OrderId)">整单发货
            </el-button>
            <el-button v-if="
                (scope.row.OrderStatus == 30 || scope.row.OrderStatus == 31) &&
                IsExpressOrder
              " class="button-op-reset" size="small" @click="handleSend1(scope.row.OrderId)">组合发货
            </el-button>
            <el-button v-if="scope.row.OrderStatus == 0 && IsCancelOrder" class="button-op-del" size="small"
              @click="handleCannel(scope.row.OrderId)">取消
            </el-button>
            <el-button v-if="
              ...
              " class="button-op-del" size="small" @click="handleRefundAll(scope.row.OrderId)">全额退款
            </el-button>
            <el-button v-if="
                (scope.row.OrderStatus == 20 ||
                  scope.row.OrderStatus == 30 ||
              ...
              " class="button-op-del" size="small" @click="handleRefund(scope.$index, scope.row.OrderId)">部分退款
            </el-button>
            ...
            <el-button class="button-op-reset" size="small" @click="handleHistoryBtn(scope.row.OrderId)">日志
            </el-button>
            <el-button v-if="(scope.row.OrderStatus == 20||scope.row.OrderStatus == 10||scope.row.OrderStatus == 30||scope.row.OrderStatus == 31||scope.row.OrderStatus == 51||scope.row.OrderStatus == 40||scope.row.OrderStatus == 100)&&IsOrderPromoteFeeDeal" class="button-op-edit" size="small"
              @click="handleManualPromoter(scope.row.OrderId)">手工佣金
            </el-button>
            <el-button
              v-if="(query.search.order_status=='30,31'||scope.row.OrderStatus == '31,40'||scope.row.OrderStatus == '100')&&loginUser.UnitName=='礼牛严选'"
              class="button-op-reset" size="small" @click="SynOrderLogClick(scope.row.OrderId)">从云中鹤同步快递单
            </el-button>
          </div>
        </template>
      </el-table-column>
    ...
    <!-- 发货弹出框 -->
    <el-dialog title="发货" v-model="deliverGoodsVisible" width="38%" :close-on-click-modal="false">
    ...
    <!-- 批量操作审核 -->
    <el-dialog title="手工发券" v-model="sendTicketVisible" width="38%" :show-close="false">
      <el-input type="textarea" maxlength="100" :rows="4" show-word-limit placeholder="请输入备注"
        v-model="textareaSendTicket">
      </el-input>
      <template #footer>
        <span class="dialog-footer">
          <el-button class="button1" :loading="sendLoading" @click="sendTicketModalBtn">确 认</el-button>
          <el-button class="button2" @click="sendTicketVisible = false">关 闭</el-button>
        </span>
      </template>
    </el-dialog>
    ...
    <!-- 显示订单操作记录 -->
    <el-dialog title="操作记录" v-model="hisesVisible" :show-close="false" width="80%" height="300">
      <div style="width:100%;height:18px"></div>
      <el-table :header-cell-style="{ 'text-align': 'center' }" v-loading="tableHistoryLoading" border
      ...
      <template #footer>
        <span class="dialog-footer">
          <el-button class="button2" @click="hisesVisible = false">关 闭</el-button>
        </span>
      </template>
    </el-dialog>
    ...
    <el-dialog title="修改收件信息" v-model="receivingVisible" width="40%">
      ...
      <el-row>
        <el-col :span="22">
          <el-form ref="formUpdateRef" :model="formUpdate_modal" :rules="rules_modal" label-width="120px">
            ...
            <el-form-item label="收件城市">
              <el-row>
                <el-col :span="7">
                  <el-select v-model="formUpdate_modal.ContactProvinceAreaId" filterable style="width: 100%"
                ...
                <el-col :span="7" :offset="1">
                  <el-select v-model="formUpdate_modal.ContactCityAreaId" filterable style="width: 100%"
                ...
                <el-col :span="8" :offset="1">
                  <el-select v-model="formUpdate_modal.ContactDistrictAreaId" filterable style="width: 100%"
                    placeholder="请选择" @change="changeDistricts($event, 'Contact')">
            ...
            <el-form-item label="备注">
              <el-input v-model="formUpdate_modal.OrderRemark" maxlength="100" :rows="4" show-word-limit
                type="textarea">
              </el-input>
            </el-form-item>
          </el-form>
    ...
    <!-- 部分退款弹出框 -->
    <el-dialog title="退款信息" class="datatable-refund" v-model="refundVisible" width="52%">
      <el-row>
        <el-col :span="24">
                ...
                <div v-for="(item, index) in scope.row.Ticketes" :key="item.index" :index="index">
                  <p>
                    <el-checkbox v-if="scope.row.Ticketes.length >= 1" v-model="item.selected"
                      @change="SelTicketIndex($event, index)" :disabled="item.TicketStatus != 2" label="" />
                    {{
          ...
          <div style="width: 100%; height: 10px"></div>
          <el-input v-model="Remark" placeholder="请输入退货原因" maxlength="100" :rows="4" show-word-limit type="textarea">
          </el-input>
        </el-col>
      </el-row>
      <template #footer>
        <span class="dialog-footer">
          <el-button class="button1" :loading="refundLoadingBtn" @click="RefundSaveModal">保 存</el-button>
          <el-button class="button2" @click="refundVisible = false">关 闭</el-button>
        </span>
      </template>
    </el-dialog>
    <!-- 整单退款 -->
    <el-dialog title="全额退款" v-model="refundAllVisible" width="38%" :show-close="false">
      <div style="width:100%;height: 18px;"></div>
      <el-input type="textarea" maxlength="100" :rows="4" show-word-limit placeholder="请输入退款理由"
        v-model="refundAllApprove">
      </el-input>
      <template #footer>
        <span class="dialog-footer">
          <el-button class="button1" :loading="refundAllLoading" @click="refundAllBtn">确 认</el-button>
          <el-button class="button2" @click="refundAllVisible = false">关 闭</el-button>
        </span>
      </template>
    </el-dialog>
              ...
              <el-row>
                ...
                <el-col :span="19" :offset="1">
                  <div class="out-bg1 flex-col justify-center">
                    ...
                    <div class="file-detail">
                      <el-upload v-model:file-list="fileInvoiceList" :limit="1" :on-remove="handleInvoiceRemove"
                        class="file-order-uploader" action="#" :http-request="SuccessInvoiceFile" :auto-upload="true"
      ...
      <template #footer>
        <span class="dialog-footer">
          <div v-if="activeInvoiceIndex == 0">
            <el-button v-if="isInvoiceDataError" class="button1" @click="isInvoiceDataError = false">重新上传</el-button>
...
const IsOrderPromoteFeeDeal = ref(false);
const BoolRight = async () => {
  ...
  if (res.Code == 200) {
    ...
    IsOrderPromoteFeeDeal.value=res.DataMap.order_promote_fee_deal;
  }
};
//取消配送单权限控制
BoolRight();
...
//同步云中鹤快递单号
const SynOrderLogClick = async (ID) => {
  let res = await SynOrderLogisticsFromHaoxiny({
    OrderId: ID,
  });
  if (res.Code == 200) {
    ElMessage.success("操作成功");
...
const IsOpenPromoteApi= async ()=>{
  let res=await IsOpenPromote({
    MallUnitId:loginUser.UnitId
  })
  if(res.Code==200){
    IsOpen.value=res.DataMap;
  }
}
////////////////////////////////////////////////////////////////////////////////////////////////////
init();
...
watch(
  ...
  (data) => {
    if (data.meta.title == "自有订单" || data.meta.title == "礼牛订单") {
