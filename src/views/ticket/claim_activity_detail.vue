              ...
              <div class="list-info-detail-info">
                <el-row>
                  <el-col :span="2">
                    <div class="card-img">
                      <img
                        @click="handleOpenQr(FormData.TakeWxxcxQRCode)"
                        :src="FormData.TakeWxxcxQRCode"
                  ...
                  <el-col :span="4">
                    <div
                      class="list-info-detail-info-th flex-col justify-center items-center"
                    >
                      <div class="list-info-detail-info-td">编码</div>
                  ...
                  <el-col :span="8">
                    <div
                      class="list-info-detail-info-th flex-col justify-center items-center"
                    >
                      <div class="list-info-detail-info-td">标题</div>
                      <div class="list-info-detail-info-tr">
                        {{ FormData.BillTitle }}
                      </div>
                    </div>
                  ...
                  <el-col :span="4">
                    ...
                    >
                      <div class="list-info-detail-info-td">
                        领取进度({{ (FormData.TakenCount/FormData.TicketCount).toFixed(2)*100 }}%)
                      </div>
                      <div
                        class="list-info-detail-info-tr"
                        style="width: 100%; text-align: center"
                      >
                        <div style="height: 15.67px"></div>
                        <el-progress
                          :percentage="FormData.TakePercent"
                          :show-text="false"
                        />
                      </div>
                    </div>
                  ...
                  <el-col :span="3">
                    <div
                      class="list-info-detail-info-th flex-col justify-center items-center"
                    >
                      <div class="list-info-detail-info-td">未领(张)</div>
                  ...
                  <el-col :span="3">
                    <div
                      class="list-info-detail-info-th flex-col justify-center items-center"
                    >
                      <div class="list-info-detail-info-td"></div>
                      <div class="list-info-detail-info-tr send-number">
                        <el-button
          ...
          <el-tab-pane label="发券信息" name="first">
            <div v-if="true">
              <el-row>
                ...
                >
                <el-table
                  :data="form.TicketTemplateDetails"
                  border
                  class="table"
                  ...
                  >
                    <template #default="scope">
                      <div v-if="scope.row.GoodsQty > 1">
                        {{
                          scope.row.GoodsQty + "选" + scope.row.ExchangeGoodsQty
                        }}
                      </div>
                      <div v-else>单选卡</div>
                    </template>
          ...
          <el-tab-pane label="领取日志" name="second">
              ...
              <el-form label-width="1px">
                <el-row>
                  <el-col :span="5">
                    <el-form-item label="">
                      <el-input
                        v-model="queryStaffs.search.search_keyword"
                        clearable
                  ...
                  <el-col :span="5" :offset="1">
                    <el-form-item label="">
                      <el-date-picker
                        v-model="queryStaffs.search.time"
                        type="daterange"
                        range-separator="To"
                  ...
                  <el-col :span="5" :offset="1">
                    <el-form-item label="">
                      <el-select
                        v-model="queryStaffs.search.take_status"
                        clearable
                        style="width: 100%"
                        placeholder="请选择状态"
                      >
            ...
            >
              ...
              <el-table-column label="领取信息" align="center">
                <template #default="scope">
                  <div v-if="scope.row.TakeTicketes">
                    <div
                      v-for="(obj, index) in scope.row.TakeTicketes"
                      :key="index"
                    >
                      <p>
                        {{ obj.TicketTemplateTitle + "-" + obj.TicketCode }}
                      </p>
                    </div>
                  </div>
                </template>
              ...
              <!-- <el-table-column label="操作" align="center" width="150">
                <template #default="scope">
                  <div class="button-mgl">
                    <el-button
                      v-if="scope.row.StaffCell && scope.row.TakeTicketes"
                      class="button-op-edit"
                      size="small"
                      @click="
                        handleSendCell(scope.row.BillId, scope.row.StaffCell)
                      "
                      >短信通知
                    </el-button>
                    <el-button
                      v-if="scope.row.StaffEmail && scope.row.TakeTicketes"
                      class="button-op-edit"
                      size="small"
                      @click="
                        handleSendEmail(scope.row.BillId, scope.row.StaffEmail)
                      "
                      >邮件通知
                    </el-button>
                  </div>
            ...
            <div class="pagination">
              <el-pagination
                ...
                v-model:page-size="queryStaffs.PageRowCount"
                :page-sizes="[15, 50, 100]"
                :total="pageTotalStaffs"
                @current-change="handlePageChangeStaffs"
                @size-change="handleSizeChangeStaffs"
              ></el-pagination>
            </div>
          </el-tab-pane>
        </el-tabs>
...
import {
  SearchTicketBillTakeStaffs,
  GetTicketBillAct,
  SendTicketTakeSMSNotify,
  SendTicketTakeEmailNotify,
} from "@/api/bill.js";
...
const getData = async () => {
  ...
  if (queryStaffs.search.time && queryStaffs.search.time.length > 0) {
    queryStaffs.search.take_time_begin = common.dateFormat(
      "YYYY-mm-dd",
      queryStaffs.search.time[0]
    );
    queryStaffs.search.take_time_end = common.dateFormat(
      "YYYY-mm-dd",
      queryStaffs.search.time[1]
    );
  } else {
...
const GetTicketBillTakeByID = async (ID) => {
  ...
  if (res.Code == 200) {
    ...
    form.EmailNotifyAndSmsNotify = form.EmailNotify
      ? form.EmailNotify + "、"
      : "" + form.SmsNotify;
    ...
    FormData.BillTitle = res.DataMap.BillTitle;
    FormData.BillCode = res.DataMap.BillCode;
    ...
    FormData.TakeWxxcxQRCode = res.DataMap.TakeWxxcxQRCode
      ? res.DataMap.TakeWxxcxQRCode
      ...
      tableLogData.value.forEach(function (item, i) {
        multipleTableRef.value.forEach(function (multItem, j) {
          if (
            ...
            multipleTable.toggleRowSelection(tableLogData.value[i], true); //这步操作是让  页面显示选中的数据
          }
        });
      });
    }
...
const filterTableData = computed(() =>
  tableLogData.value.filter(
    (data) =>
      ...
      data.TakeVipName.toLowerCase().includes(
        query.search.search_keyword.toLowerCase()
      ) ||
      data.TakeVipCell.toLowerCase().includes(
        query.search.search_keyword.toLowerCase()
      ) ||
      data.TakeTime.toLowerCase().includes(
        query.search.search_keyword.toLowerCase()
      )
  )
);
